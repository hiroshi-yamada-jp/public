...
# グラフ操作関数
# ==========================
def create_multigraph(graph_id: int, name: str, device_type: str = "") -> nx.MultiGraph:
    """無向マルチグラフの新規作成
    ...
    G.graph["id"] = graph_id
    G.graph["name"] = name
    G.graph["device_type"] = device_type
    return G
...
def update_graph_attrs(G: nx.MultiGraph, new_attrs: dict[str, Any]) -> None:
    """グラフ属性を追加または更新する
...
# ノード操作関数
# ==========================
def find_target_node(G: nx.MultiGraph, node_id: int) -> dict | None:
    """指定ノードを取得
...
def add_node(G: nx.MultiGraph, name: str, node_id: int, description: str = "") -> dict | None:
    """単一ノードの登録
    ...
    Args:
        ...
        name: ノード名
        node_id: ノードID
        description: 説明(指定省略可)
    Returns:
        dict: 登録内容 / None: None: 登録失敗（追加対象が登録済)
    """
    if find_target_node(G, node_id=node_id) is not None:
        print(F"ERROR: 指定IDのノードは登録済:{node_id=}")
        return None
    G.add_node(node_id, name=name, description=description)
    return find_target_node(G, node_id=node_id)
...
def add_nodes_bulk(G: nx.MultiGraph, node_list: list[dict]) -> list[dict] | None:
    ...
    # ノードの一括登録
    for node in node_list:
        add_node(G, name=node["name"], node_id=node["id"], description=node.get("description", ""))
    return get_nodes(G)


def get_nodes(G: nx.MultiGraph, minimal: bool = False) -> list[dict]:
    """グラフに登録されているノード一覧の取得

    Args:
        G (nx.MultiGraph): 対象グラフ
        minimal: True: 名前とIDのみ取得 / False(default): 全属性取得
    Returns:
        list[dict]: ノード属性の一覧
    """
    result = []
    for node_id, attrs in G.nodes(data=True):
        if minimal:
            result.append({"id": node_id, "name": attrs.get("name", "")})
        else:
...
def get_adjacent_nodes(G: nx.MultiGraph, node_id: int) -> list[dict]:
    ...
    Args:
        ...
        node_id: 対象ノードID
    Returns:
        list[dict]: 隣接ノード一覧（各辞書に隣接ノードの'node_id' を含む）/ 空リスト:指定ノードまたは隣接ノードが存在しない場合
    """
    res = []
    if not node_id in G.nodes:
        print(F"ERROR: 指定IDのノードは未登録:{node_id=}")
...
# エッジ操作関数
# ==========================
def _generate_edge_key(node1_id: int, route1_id: int, node2_id: int, route2_id: int) -> str:
    """エッジ生成情報からエッジキーを生成する"""
    edge_key = f"{node1_id}:{route1_id}--{node2_id}:{route2_id}"
    return edge_key


def find_target_edge(G: nx.MultiGraph, node1_id: int, route1_id: int, node2_id: int,
                     route2_id: int) -> dict | None:
    """指定エッジ情報に対応するエッジが孫座すれば、その属性を取得する
    ...
    """
    key = _generate_edge_key(node1_id, route1_id, node2_id, route2_id)
    try:
        # node1_id と node2_id の間に存在するエッジ群からkey に一致するエッジを選ぶ
        attrs = G.edges[node1_id, node2_id, key]
        return {
            "node1_id": node1_id,
            "route1_id": route1_id,
            "node2_id": node2_id,
...
def add_edge(G: nx.MultiGraph, node1_id: int, route1_id: int, node2_id: int, route2_id: int) -> dict | None:
    ...
    Note:
        - 同一ノード間のエッジでも route ID が異なれば別エッジとして扱う
    """
    # 恩地エッジが登録済の場合、登録失敗
    if find_target_edge(G, node1_id, route1_id, node2_id, route2_id) is not None:
    ...
    # 登録した内容を返す
    return find_target_edge(G, node1_id, route1_id, node2_id, route2_id)


def add_edges_bulk(G: nx.MultiGraph, edge_list: list[dict]) -> list[tuple]:
    """複数エッジを一括登録する
    ...
    for edge in edge_list:
        ...
        if find_target_edge(G, node1_id, route1_id, node2_id, route2_id):
            print(F"ERROR: 指定エッジは登録済:{node1_id=}\t{node2_id=}")
            return res
    # 各指定エッジの登録
    for edge in edge_list:
        node1_id, route1_id = edge["node1_id"], edge["route1_id"]
        node2_id, route2_id = edge["node2_id"], edge["route2_id"]
        add_edge(G, node1_id, route1_id, node2_id, route2_id)
    return get_edges(G)


def get_edges(G: nx.MultiGraph, minimal: bool = False) -> list[dict]:
    """エッジ一覧を取得する
    ...
    # G.edges(keys=True, data=True)のkeys=Trueでkey取得が可能となり、data=Trueで属性(attrs)取得が可能となる
    for node_id1, node_id2, key, attrs in G.edges(keys=True, data=True):
        edge_info = {"node1_id": node_id1, "node2_id": node_id2, "key": key}
        if minimal:
...
def update_edge_attrs(G: nx.MultiGraph, node1_id: int, route1_id: int, node2_id: int, route2_id: int,
...
def get_simple_path(G: nx.MultiGraph, node1_id: int, node2_id: int) -> list[list[int]]:
    """指定されたノード間の全ての接続パスを取得する
    指定ノードに複数のパスが存在する場合の戻り値の例: 下記の例では3つのパスが存在し、そのノードIDをそれぞれのリストで示している
        get_simple_path(G, 1,6)=[[1, 5, 6], [1, 2, 3, 4, 5, 6], [1, 9, 8, 7, 6]]
    ...
    Args:
        ...
        node2_id (int): 終了ノードID
    Returns:
        list[list[int]]: 見つかった全てのパス（パスが存在しない場合 空リストを返す）
    """
    try:
